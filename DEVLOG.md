...
| 4-33 | 路由规则模板扩展 | `routerService.getRuleTemplates()` 新增 8 个预置规则模板：编码优化 / 复杂推理 / 测试生成 / 文档生成 / 严格代码审查等场景 | ✅ |
| 4-34 | 规则模板 API | `GET /api/router/rules/templates` 新增接口，前端可直接获取所有预置模板 | ✅ |
| 4-35 | 路由模拟页面增强 | 输入框下方新增 8 个常用场景示例按钮（Bug修复/代码生成/安全审查等），点击快速填入；策略下拉框预先选中该场景对应的推荐策略 | ✅ |
| 4-36 | 路由历史记录展示 | 路由模拟 Tab 新增"路由决策历史"面板，展示最近 20 条路由决策（任务类型 / 用户意图摘要 / 推荐模型 / 决策时间） | ✅ |
| 4-37 | 路由规则快速添加 | 路由规则 Tab 新增"预置规则模板（一键添加）"区块，8 个模板以卡片形式展示，已存在的规则显示"✓ 已存在"禁用状态 | ✅ |
| 4-38 | Dashboard 路由统计 | `dashboardService.getRouterStats()` + `GET /api/dashboard/router-stats` 接口新增；Dashboard 新增路由统计卡片行（总数/活跃模型/TOP模型/TOP策略） | ✅ |
| 4-39 | Dashboard 路由面板 | Dashboard 图表区新增"🧭 路由决策统计"全屏面板，展示任务类型分布柱状图 + 策略使用标签 + 模型使用排行 | ✅ |
...
| `src/server/services/ruleService.ts` | ✏️ 修改 | 计数更新 + inject_prompt 逻辑 + `acknowledgeAllAlerts` + 8 个预置规则模板 |
| `src/server/services/promptInjectionService.ts` | 🆕 新建 | 完整提示生成 + 存储服务 |
| `src/server/services/dashboardService.ts` | ✏️ 修改 | 新增 getRuleStats + getRouterStats |
| `src/server/services/routerService.ts` | ✏️ 修改 | 新增 getRuleTemplates() 8 个场景模板 |
| `src/server/routes/promptInjection.ts` | 🆕 新建 | GET/PATCH/DELETE 路由 |
| `src/server/routes/router.ts` | ✏️ 修改 | 新增 `GET /api/router/rules/templates` |
| `src/server/routes/dashboard.ts` | ✏️ 修改 | 新增 `GET /api/dashboard/router-stats` |
...
| `src/client/src/pages/Dashboard.tsx` | ✏️ 修改 | 规则统计 + 提示统计卡片 + 告警 Banner + 已确认/总计统计卡片 + 路由决策统计（卡片行+全屏面板） |
| `src/client/src/pages/Alerts.tsx` | ✏️ 修改 | 过滤器修复 + 一键处理按钮 + UI 样式增强（待处理标签/确认处理按钮/已确认标签） |
| `src/client/src/pages/ModelRouting.tsx` | ✏️ 修改 | 常用场景示例按钮 + 路由历史记录 + 规则模板快速添加 |
